...
export const PDFReport: React.FC<PDFReportProps> = ({ content, workflowData }) => {
  ...
  const contentSource = isRealAI ? "🤖 Real Claude 4 Opus Content" : "⚠️ Static Template (Deploy to Production)";
  ...
  const renderCoverPage = () => (
    <Page size="A4" style={styles.coverPage}>
      ...
      <View style={styles.footer}>
        <Text>Generated {new Date().toLocaleDateString()} | Powered by Claude 4 Opus AI | {contentSource}</Text>
      </View>
    </Page>
  );
  ...
  const renderDynamicContent = () => (
    <Page size="A4" style={styles.page}>
      {!isRealAI && (
        <View style={styles.sourceIndicator}>
          <Text style={styles.sourceText}>
            ⚠️ DEVELOPMENT MODE: Showing static template. Deploy to production for real Claude 4 Opus content.
          </Text>
        </View>
      )}
      
      <Text style={styles.sectionTitle}>{executiveSummary.title || 'Executive Summary'}</Text>
      <Text style={styles.contentText}>{executiveSummary.overview || 'Implementation overview will be generated by Claude 4 Opus.'}</Text>
      ...
      <Text style={styles.contentText}>{technicalSpec.architectureOverview || 'Technical architecture will be detailed by Claude 4 Opus.'}</Text>
          ...
          {codeExamples.slice(0, 2).map((example, index) => (
            <View key={index}>
              ...
              <View style={styles.codeBlock}>
                <Text style={styles.codeText}>{example.code || '// Code will be generated by Claude 4 Opus'}</Text>
              </View>
            </View>
          ))}
        </>
    ...
    <Page size="A4" style={styles.page}>
      ...
      {implementationPlan.phases && implementationPlan.phases.length > 0 && implementationPlan.phases.map((phase, index) => (
        <View key={index} style={styles.metricBox}>
          ...
          <Text style={styles.metricValue}>{phase.description || 'Phase details will be provided by Claude 4 Opus'}</Text>
