...
// PandaDoc webhook endpoint for local development
app.post('/webhook/pandadoc-signed', async (req, res) => {
  try {
    console.log('🔔 Local PandaDoc webhook received:', {
      headers: req.headers,
      body: req.body
    });
    ...
    if (event_type === 'document_completed' || event_type === 'recipient_completed') {
      ...
      const documentData = {
        documentId: data?.id || 'test-doc-id',
        documentName: data?.name || 'Test Document',
      ...
      console.log('🚀 Local webhook: Simulating automation workflow...');
      ...
      return res.status(200).json({
        ...
        documentId: data?.id,
        documentData,
        note: 'This is the local development webhook. Deploy to Vercel for production.'
      });
    }
    ...
    console.error('❌ Error processing local PandaDoc webhook:', error);
