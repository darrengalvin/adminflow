...
import FirebaseCredentialManager from './components/FirebaseCredentialManager';
...
const AppContent: React.FC = () => {
  ...
  // Determine authentication state based on which system we're using
  const isAuthenticated = USE_FIREBASE 
  ...
  console.log('🔐 Auth Debug:', {
    ...
    firebaseUser: firebaseAuth?.user?.email,
  ...
  if (!isAuthenticated) {
    return USE_FIREBASE ? <FirebaseLoginForm /> : <LoginForm />;
  }
  ...
  const renderContent = () => {
    switch (currentSection) {
      ...
      case 'admin-workflow':
        return <AdminWorkflow onBack={() => setCurrentSection('home')} />;
      case 'credentials':
        return USE_FIREBASE ? 
          <FirebaseCredentialManager onBack={() => setCurrentSection('home')} /> :
          <CredentialManager onBack={() => setCurrentSection('home')} />;
      case 'reports':
        return <Reports />;
      case 'report-history':
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      {isMobile ? (
        <div className="flex flex-col min-h-screen">
          <div className="flex-1 overflow-auto pt-12"> {/* Add top padding for activity bar */}
        ...
        <div className="flex">
          <Sidebar 
            ...
            onNavigate={setCurrentSection}
          />
          <div className="flex-1 ml-64 pt-12"> {/* Add top padding for activity bar */}
            {renderContent()}
          </div>
        </div>
      )}
